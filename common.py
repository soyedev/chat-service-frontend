...
# 로컬에서는 환경변수 없이 기본값으로 동작하고, Streamlit Community Cloud 에 배포하면
# secrets.toml 의 BACKEND_URL 이 이 기본값을 덮어쓴다.
# secrets.toml 이 아예 없으면(로컬) st.secrets 접근에서 예외가 나므로 감싸서 처리한다.
try:
